...
import { analyseEggs, validOpeningBlock, type EggAnalysis } from "../eggs";
...
export interface ArticleOptions {
  ...
  /**
   * Whether this chapter is the document's own first chapter.
   *
   * `.opening` "belongs to the first block of the first chapter"
   * (`05-internals.md` section 3): a document has one opening, not one per
   * chapter, so a composer rendering more than one chapter — `publish/
   * build.ts`, `preview.ts` — passes `false` for every chapter but the
   * first. Defaults to `true`, which is what makes a single-chapter caller
   * — a test, or this module used on its own — the ordinary case: a chapter
   * rendered alone is the only chapter there is, so it is its document's
   * first.
   */
  readonly isFirstChapter?: boolean;
}
...
interface Article {
  ...
  readonly chapter: Chapter;
  /** This chapter's easter eggs, matched once for both the flow and the tray. */
  readonly eggs: EggAnalysis;
  /** The one `.opening` block a reader is shown, or null where this chapter has none. */
  readonly validOpening: Block | null;
}
...
/**
 * The once-only opening quotation, in the flow.
 *
 * `article.eggs`, egg matching aside, only ever holds one `.opening` block
 * worth showing: [`validOpeningBlock`] already answered the position question
 * — the document's own first chapter's own first block — so this renders
 * exactly that one instance and nothing for any other `.opening` div the
 * canon still routes here (`placementOf` knows only the construct, not where
 * it sits). Reference equality against `article.validOpening` is safe because
 * `renderBlocks` and `renderOneBlock` never copy a block before reaching this
 * switch; a misplaced `.opening` is the same object seen from the same array,
 * never a clone that identity would miss.
 *
 * The markup itself is plain flow content, with no `hidden` attribute: a
 * reader with no script sees the quotation as an epigraph, in place, every
 * time (`itd-2609051336110536`, degrade). `article-eggs.js` is what takes it
 * out of the flow and into a modal, and remembers that it did — nothing here
 * writes to storage, reads storage, or assumes a script will ever run.
 */
...
/**
 * An easter egg's own content, kept out of the flow in a `<template>`.
 *
 * A `<template>`'s content is inert until a script reads it — never painted,
 * never reachable by Tab, never announced — which is the whole of how "the
 * block's content appears nowhere in the flow" holds with no script-specific
 * branch in this renderer: a browser with CSS and no JavaScript at all still
 * sees nothing here, the same as one that never loaded `article-eggs.js`.
 * It is also, deliberately, still markup: phase 6's print rendering has
 * something to read once it exists (spc-2609061318159422's own scope note),
 * because a `<template>` is present in the document tree a print renderer
 * would walk, not generated by a script that print never runs.
 *
 * `id` is the same string the marker's own `data-egg` names
 * (`html.ts`'s `eggMarker`), so `article-eggs.js` finds one from the other
 * with a plain `getElementById` and no id arithmetic of its own. An orphan
 * block — matched to no marker — renders nothing at all: `article.eggs.
 * matched` only holds a block once something in the chapter named it.
 */
function renderEgg(block: Block, article: Article): string {
  const id = block.attributes.id;
  const match = id === null ? undefined : article.eggs.matched.get(id);
  if (match === undefined || match.block !== block) return "";
  return `<template${attributes([
    ["id", `${article.idPrefix}egg-${id}`],
    ["data-egg-label", match.label],
  ])} class="egg-content">${renderBlocks(block.children, article)}</template>`;
}
...
function renderOneBlock(block: Block, article: Article): string {
  ...
  switch (placementOf(block, "article")) {
    ...
    case "modal":
      // `.opening`: the once-only quotation, itd-2609051335518134 (map #12).
      return renderOpening(block, article);
    case "collectable":
      // `.egg`: an easter egg's content, itd-2609051335518134 (map #12).
...
export function renderArticle(
  ...
  const eggs = analyseEggs(chapter);
  const eggLabels = new Map([...eggs.matched].map(([id, match]) => [id, match.label]));
  const article: Article = {
    ...
    chapter,
    eggs,
    validOpening: validOpeningBlock(chapter, options.isFirstChapter ?? true),
  };
  const contents =
