...
Every screen rendering is complete and readable at iPhone, iPad, and
desktop widths, with vertical scrolling only. Nothing requires horizontal
scrolling of the page or a pinch to read.
...
Phase 1, with the deck, and it governs each rendering as it lands: the
article and its reader controls in phase 2, the single file in phase 5,
the published pages throughout. The paged PDF is measured by its own
layout rather than by viewport width.

## How A Spec Proves It

- Given the article at an iPhone width, When Carol reads to the end,
  Then the page's scroll width equals the viewport width at every point
  and she scrolls in one direction only.
- Given a citation whose full reference sits in the margin at desktop
  width, When the viewport narrows to iPhone width, Then the note folds
  into the flow directly after the paragraph that made it and remains
  readable.
- Given a wide table and a long unbroken line of code at iPhone width,
  When they are rendered, Then each scrolls within its own container and
  the page around it does not.
- Given the deck at iPhone, iPad, and desktop widths, When Bob follows
  from the back row on his phone while the projector shows the same
  slide, Then every slide's content is present and legible at each width,
  with nothing cropped.
- Given the reader controls and the navigation at iPhone width, When
  Carol opens each, Then each opens within the viewport and closes
  without the page moving sideways.
- Given any rendering measured at all three widths, When a single element
  overflows the viewport, Then the discipline fails, whatever the rest of
  the page does.
