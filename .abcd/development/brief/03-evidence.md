...
| An export panel that writes the deck or the article as a folder Alice chooses, through the one builder the publish path uses | The single HTML file as the only local artefact | A folder can be handed over or opened from a memory stick; the single file remains the one-file answer and the published link the shareable one |
