...
| No file in either folder holds an absolute path, a machine name or a user name; every reference between the folder's files is relative | `an_exported_folder_carries_no_absolute_path` (walks the written folder, refusing the temporary root, the home folder's own prefix, `file:` and the host name); `src/publish/build.test.ts` › "names nothing above the folder it is written into" |
