...
| Kill a line and yank it back twice; act on a region between mark and point | `src/emacs-keys.test.ts` › "kills to the end of the line", "yanks what was killed back", "sets the mark…", "kills the region between the mark and the point" (exist), extended to yank twice |
...
| Unsaved text is not on disk; the modeline marks it; switching or closing asks; saving differs only by the paragraph | `src/emacs-keys.test.ts` › "saves the open chapter on C-x C-s", "asks before unsaved edits are lost…", "holds a close back while the chapter is dirty…" (exist) |
